...
- JSON Adapter: `OrderRepository` 신규 (Phase 0에서 결정한 저장 구조 적용)
...
### 구현 범위
- Application: `ApproveOrderUseCase`(재고 판정 분기), `RejectOrderUseCase`
- Domain: 재고 판정 로직(충분/부족), Order 상태 전이 규칙

### 완료 조건
- 재고 충분 시 CONFIRMED 전환, 재고 부족 시 PRODUCING 전환 + ProductionJob 생성이 검증됨
...
### 구현 범위
- Domain: `ProductionJob`, `ProductionSchedule.Reconcile(jobs, now)`(순수 함수, FIFO 스케줄+
  catch-up 규칙)
- Application: `SettleProductionUseCase`, `SettlingUseCaseDecorator`, `IClock` 포트
- JSON Adapter: `ProductionQueueRepository`
