...
- **저위험 Phase (1, 2, 3, 5, 6)**: 이미 검증된 PoC 패턴을 그대로 가져다 쓰거나(1/2/6: ConsoleMVC/
  DataPersistence 패턴 재사용) 상대적으로 단순한 로직(3/5: 재고 판정 분기, 집계 조회)이라 리스크가
  낮다. 이 그룹에서는:
  ...
  - 커밋 메시지는 COMMIT_CONVENTION.md의 60자 규칙을 문장이 자연스럽게 끊기는 선에서만 지키고,
    억지로 wrapping하지 않는다.
...
- **고위험 Phase (4, 7)**: 위 완화를 적용하지 않고 이 문서 본문의 SDD Phase 진행 규칙을 문자
  그대로 지킨다(각 화살표 지점마다 사용자 승인).
  - Phase 4(생산 라인과 정산): PRD.md 5.5의 FIFO+catch-up+멱등성 규칙이 이 프로젝트에서 가장
    복잡하고 실수 가능성이 크기 때문.
  - Phase 7(통합 리팩터링과 최종 Review): 전체 계층을 가로지르는 마지막 게이트이기 때문.
