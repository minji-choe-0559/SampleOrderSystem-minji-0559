...
- **저위험 Phase (1, 2, 3, 5)**: 이미 검증된 PoC 패턴을 그대로 가져다 쓰거나(1/2/5: ConsoleMVC/
...
- **고위험 Phase (4, 6)**: 위 완화를 적용하지 않고 이 문서 본문의 SDD Phase 진행 규칙을 문자
  그대로 지킨다(각 화살표 지점마다 사용자 승인).
  - Phase 4(생산 라인과 정산): PRD.md 5.5의 FIFO+catch-up+멱등성 규칙이 이 프로젝트에서 가장
    복잡하고 실수 가능성이 크기 때문.
  - Phase 6(통합 리팩터링과 최종 Review, 기존 Phase 7): 전체 계층을 가로지르는 마지막 게이트이기
    때문.
...
줄지만 각 커밋 메시지의 설명 밀도로 SDD/TDD 실천 근거를 보존한다. 고위험 Phase(4, 6)는 이 압축을
적용하지 않고 RED/GREEN/REFACTOR를 계속 별도 커밋으로 남긴다.
...
보고한다(생략 금지). **아래 5개 블록은 고위험 Phase(4, 6)에서는 각 화살표 지점마다 순서대로 별도
승인받는 그대로 적용한다. 저위험 Phase(1/2/3/5)에서는 5개를 각각 승인받지 않고, Phase 완료
...
- 이번 커밋에 포함될 변경 파일 목록과 Breaking Change 여부
- 제안 커밋 메시지 전문(COMMIT_CONVENTION.md 형식)
...
- Format Check (clang-format, 검사 전용, 자동 수정 금지)

## Git 작업 규칙

### 커밋 수행 가능 범위

- 사용자의 명시적 승인 없이 Commit, Push, Merge, PR 생성을 하지 않는다. **Phase 완료 시점마다
  승인이 필요**하며, DataPersistence류의 "Step 구간 사전 승인" 방식을 이 저장소에는 적용하지
  않는다.
