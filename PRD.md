...
- **시료 검색**: 이름 등 속성으로 특정 시료를 검색한다. (검색 가능 필드 범위는 8장 "결정 보류
  사항" 참고)
...
| `sampleId` | 대상 Sample |
| `quantity` | 실 생산량. `ceil(부족분 / 수율)`로 Job 생성 시점에 한 번 확정, 이후 변경하지 않는다. |
...
- **SOLID**: 애그리게잇별 Repository 분리(`SampleRepository`/`OrderRepository`/
  `ProductionQueueRepository`), Domain은 Port(인터페이스)에만 의존.
- **기존 PoC 재사용**:
...
`docs/DATA_CONTRACT.md` v1을 그대로 따른다. Order/ProductionJob을 위한 저장 구조 확장은 8장의
결정 보류 사항이며, 확정되는 즉시 이 문서와 `docs/DATA_CONTRACT.md`(신규 작성)에 반영한다.

## 8. 결정 보류 사항 (임의로 정하지 않음)

- Order/ProductionJob을 어떤 파일/저장소 구조로 나눌지 (같은 파일에 배열 추가 vs 별도 파일 vs
  별도 JSON 엔진 재사용 방식)
- 재고 "여유/부족/고갈" 판정 임계치(정량 기준)
- 생산 라인 개수/구조(단일 라인 고정 여부, 시료별 전용 여부) — PPT는 "하나의 생산 라인은 시료를
  하나씩 생산"만 명시, 개수는 불명확
- 시료 검색 가능 필드 범위(이름 외 추가 여부)
- 주문번호/시료ID 채번 규칙
- 고객 관리 방식(자유 텍스트 vs 고객 마스터 엔터티)
