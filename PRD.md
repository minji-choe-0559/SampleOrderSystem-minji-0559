...
| `sampleCode` | 대상 Sample 참조. Order가 내부 Guid(`orderId`)가 아니라 사용자 대면 식별자
  `sampleCode`로 Sample을 참조하는 것과 동일한 방식으로 통일한다(실제 구현된 `Order.sampleCode`
  참고, 기존 표기 `sampleId`에서 변경). |
...
> **결정 사항(확정)**: 생산 라인은 시스템 전체에 **1개만 운영**한다(단일 고정). 모든 시료
> 종류의 `ProductionJob`이 이 하나의 FIFO 큐를 공유하며, 시료별 전용 라인을 두지 않는다.
> 아래 5.5.2의 스케줄 계산도 이 단일 큐 전제로 서술되어 있다.
...
공유 JSON 문서에 추가 배열(`orders`, `productionJobs`)로 확장**한다. 확장된 계약은
[docs/DATA_CONTRACT.md](docs/DATA_CONTRACT.md)에 반영되어 있다.
